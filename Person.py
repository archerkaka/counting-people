...
class MyPerson:
    ...
    def going_UP(self,mid_start,mid_end):
        if len(self.tracks) >= 2:
            if self.state == '0':
                if self.tracks[1][1] > mid_end and self.tracks[-2][1] <= mid_start: 
                    self.state = '1'
                    self.dir = 'up'
                    return True
            else:
    ...
    def going_DOWN(self,mid_start,mid_end):
        if len(self.tracks) >= 2:
            if self.state == '0':
                if self.tracks[1][1] < mid_start and self.tracks[-2][1] >= mid_end: 
                    self.state = '1'
                    self.dir = 'down'
                    return True
            else:
...
##Donato kodas
    def cross_bottom(self, bottom_line):
        if self.tracks[0][1] < bottom_line < self.tracks[-1][1]: 
            if self.linija1 != 'top':
                self.linija1 = 'bottom'
    ...
    def cross_top(self, top_line):
        if self.tracks[0][1] < top_line < self.tracks[-1][1]: 
